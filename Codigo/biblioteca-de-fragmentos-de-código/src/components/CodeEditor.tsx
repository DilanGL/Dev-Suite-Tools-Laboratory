...
import { Play, Eye, Code2, Search, X, WrapText, ArrowUpDown, Sliders, Type } from 'lucide-react';
  ...
  return (
    <div className={`relative flex flex-col h-full rounded-xl overflow-hidden font-mono text-sm transition-all duration-200 border border-neutral-200/10 ${styleConfig.container}`} id="code-editor-root">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#2A2D35] bg-[#1A1D23] font-sans" id="editor-controls-bar">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className={`p-1 rounded cursor-pointer transition ${wordWrap ? 'text-blue-400 bg-[#2563eb]/20' : 'text-[#6B7280] hover:text-[#E0E0E0] hover:bg-[#1F2229]'}`}
      ...
      <div className="relative flex flex-1 overflow-hidden" style={{ minHeight: '260px' }}>
        ...
        <div 
          ref={gutterRef}
          className={`w-12 select-none text-right py-4 pr-3 overflow-hidden leading-relaxed ${styleConfig.lineGutter}`}
        ...
        <textarea
          ...
          autoCorrect="off"
          autoCapitalize="off"
          className={`flex-1 p-4 outline-none border-0 leading-relaxed overflow-auto leading-6 ${styleConfig.textarea} ${wordWrap ? 'whitespace-pre-wrap' : 'whitespace-pre'}`}
          style={{ 
