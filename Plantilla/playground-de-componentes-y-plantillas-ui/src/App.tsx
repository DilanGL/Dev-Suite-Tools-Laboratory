...
export default function App() {
  ...
  const [tempKey, setTempKey] = useState<string>(() => localStorage.getItem("user_gemini_key") || "");
  const [showKeyConfig, setShowKeyConfig] = useState<boolean>(() => !localStorage.getItem("user_gemini_key"));
  ...
  const handleGenerateWithAI = async (promptText: string) => {
    ...
    setServerError(null);
    setLoadingStep("Inicializando conexión segura con el núcleo Gemini...");
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Ocurrió un error al generar el componente.");
      }

      const newCompData = await response.json();
  ...
  return (
    <div className={`min-h-screen ${activeTheme.bgApp} text-slate-100 flex flex-col font-sans transition-colors duration-500 selection:bg-cyan-500/30 selection:text-cyan-200`}>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start transition-all duration-500">
        ...
        <section className={`${appLayoutMode === 'split' ? 'lg:col-span-8' : 'lg:col-span-12'} space-y-6 transition-all duration-500`}>
                ...
                filteredComponents.map((comp) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className={`p-8 ${activeTheme.bgInner} border-b ${activeTheme.borderPanel} flex flex-col items-center justify-center min-h-[160px] relative overflow-hidden transition-all duration-500`}>
                        ...
                        <div className={`relative z-10 w-full flex items-center justify-center transition-all duration-500 ${
                          ...
                          {/* We parse the edited live HTML code or the initial code */}
                          <div 
                            className="w-full flex items-center justify-center animate-fade-in"
