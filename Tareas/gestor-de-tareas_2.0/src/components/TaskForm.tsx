...
export default function TaskForm({ onSubmit, editingTask, onCancelEdit, isSidebar = false }: TaskFormProps) {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl border-4 border-lime-400 p-5 md:p-6 transition-all duration-300">
      <div className="flex items-center justify-between mb-4 border-b-2 border-dashed border-lime-100 pb-3">
      ...
      </div>      <form onSubmit={handleSubmit} className="space-y-4" id="task-submission-form">
        ...
        <div className={`flex flex-col gap-4 pt-1 ${isSidebar ? 'items-stretch' : 'md:flex-row items-stretch md:items-end'}`}>
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            <div className={`grid gap-2.5 ${isSidebar ? 'grid-cols-1 sm:grid-cols-3 md:grid-cols-1' : 'grid-cols-3'}`} id="priority-options">
              {/* Opción Menor (Verde) */}
              <button
                type="button"
              ...
              <button
                ...
                className={`py-2 px-3 rounded-2xl border-3 flex items-center justify-center gap-1 text-xs font-extrabold tracking-wide transition-all cursor-pointer ${
                  prioridad === 'media'
                    ? 'bg-amber-400 border-amber-500 text-amber-955 shadow-md scale-[1.03]'
                    : 'bg-amber-50 hover:bg-amber-100 border-amber-200 text-amber-800'
                }`}
              >
                <span className={`w-2.5 h-2.5 rounded-full ${prioridad === 'media' ? 'bg-amber-955' : 'bg-amber-500'} shrink-0`} />
                <span>Media</span>
                {prioridad === 'media' && <Check className="w-3 h-3 stroke-[3] text-amber-955" />}
              </button>
          ...
          {/* Submit Button */}
          <div className={`shrink-0 flex ${isSidebar ? 'items-stretch' : 'items-end'}`}>
            <button
              type="submit"
              id="submit-task-btn"
              className={`w-full ${isSidebar ? '' : 'md:w-auto'} px-6 py-3.5 rounded-2xl font-extrabold text-sm shadow-md flex items-center justify-center gap-2 hover:-translate-y-0.5 active:translate-y-0 text-white transition-all cursor-pointer ${
                editingTask
