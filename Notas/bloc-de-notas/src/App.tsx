...
export default function App() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#0b0d19] text-[#e2e8f0] overflow-hidden font-sans">      {/* SIDEBAR */}
      <aside className="w-80 h-full bg-[#151a30] border-r border-[#1e293b] flex flex-col shrink-0 select-none">
        ...
        {/* Footer info stats */}
        <div className="p-4 border-t border-[#1e293b] bg-[#0b0d19]/40 flex items-center justify-between text-[11px] font-mono text-zinc-500">
          <span className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 animate-pulse inline-block" />
            Entorno Activo
          </span>
          <span>v2.2</span>
        </div>
      ...
      <main className="flex-1 h-full flex flex-col bg-[#0b0d19] overflow-hidden min-w-0">
        {activeNote ? (
          <div className="h-full flex flex-col min-w-0" id="active-note-workspace">
            ...
            <div className="px-8 py-2.5 bg-[#151a30]/40 border-b border-[#1e293b] flex flex-wrap items-center justify-between gap-3 shrink-0">
              
              {/* Display View modes */}
              <div className="flex items-center bg-[#0b0d19] p-0.5 rounded-lg border border-[#1e293b]">
                <button
                  id="btn-view-edit"
                  onClick={() => setViewMode('edit')}
                  className={`flex items-center gap-2 px-3 py-1 text-xs rounded-md transition-all ${
                    viewMode === 'edit'
                      ? 'bg-[#1e293b] text-purple-400 shadow-sm font-medium'
              ...
              <div className="w-full h-full flex-1 min-h-0">
                {viewMode === 'edit' && (
                  <textarea
                    ...
                    placeholder="// Empieza a escribir código, tokens o anotaciones aquí..."
                ...
                {viewMode === 'preview' && (
                  <div className="w-full h-full p-2 bg-transparent rounded-xl overflow-y-auto custom-scrollbar">
                ...
                {viewMode === 'split' && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full min-h-0">
                    <textarea
                      id="active-note-textarea-split"
                      value={activeNote.content}
                      onChange={(e) => handleNoteContentChange(e.target.value)}
                      placeholder="// Escribe tus notas..."
                      className={`w-full h-full min-h-0 bg-transparent border-r border-[#1e293b]/50 focus:ring-0 p-0 pr-4 font-mono resize-none text-[#e2e8f0] focus:outline-none transition-all ${getFontSizeClass(editorFontSize)}`}
                      style={{ tabSize: 4, lineHeight: '1.8' }}
                    />
