...
export default function App() {
  ...
  // Handle receiving a shared note from URL (?share=...) on initial load
  useEffect(() => {
    ...
    if (shareParam) {
      try {
        ...
        if (decoded) {
          ...
          const newNote: Note = {
            id: `shared-${Date.now()}`,
            title: sharedObj.title || 'Nota Compartida',
            content: sharedObj.content || '',
            category: sharedObj.category || 'General',
            pinned: sharedObj.pinned || false,
            images: sharedObj.images || {},
            updatedAt: Date.now()
          };
          ...
          if (stored) {
            try {
              currentNotes = JSON.parse(stored);
            } catch (e) {
              currentNotes = [...notes];
            }
          } else {
          ...
          // Avoid adding exact duplicate contents
          const isDuplicate = currentNotes.some(
            (n) => n.content === newNote.content && n.title === newNote.title
          );
          ...
          if (!isDuplicate) {
            ...
            localStorage.setItem('devspace_notes', JSON.stringify(updatedNotes));
            showToast('¡Nota compartida recibida e importada!');
          } else {
            ...
            showToast('Esta nota ya ha sido importada');
          }
        }
      } catch (e) {
        console.error('Error importing shared note:', e);
        showToast('Error al importar la nota compartida', true);
      } finally {
  ...
  const handleShareNote = () => {
    if (!activeNote) {
      showToast('No hay ninguna nota activa para compartir', true);
      return;
    }
    ...
    try {
      ...
      const compressed = LZString.compressToEncodedURIComponent(JSON.stringify(noteData));
      
      // Generate dynamic share URL based on current origin and path
      const baseUrl = window.location.origin + window.location.pathname;
      const finalShareUrl = `${baseUrl}?share=${compressed}`;

      setShareUrl(finalShareUrl);
      setShareModalOpen(true);
      showToast('Enlace de compartir generado con éxito');
    } catch (err) {
      console.error('Error compressing note:', err);
      showToast('Error al comprimir y generar enlace', true);
    }
  };
        ...
        {activeNote ? (
          <div className="h-full flex flex-col min-w-0" id="active-note-workspace">
            ...
            <header className="px-8 h-16 bg-[#151a30] border-b border-[#1e293b] flex items-center justify-between gap-4 shrink-0">
              ...
              {/* SHARE NOTE ACTION AND SAVE STATUS INDICATOR */}
              <div className="flex items-center gap-3 shrink-0" id="sync-status-indicator">
                <button
                  id="btn-share-note"
                  onClick={handleShareNote}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-md shadow-purple-900/20 active:scale-95 transition-all cursor-pointer border border-purple-500/20"
                  title="Compartir nota por medio de URL comprimida y código QR"
                >
