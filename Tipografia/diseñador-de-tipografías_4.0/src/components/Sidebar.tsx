...
import { PREMADE_FONTS, createPremadeProject } from '../utils/defaults';
import * as opentype from 'opentype.js';
import { fontToFontProject } from '../utils/fontImporter';
...
interface SidebarProps {
  ...
  backgroundProject: FontProject | null;
  onChangeBackgroundProject: (project: FontProject | null) => void;
}

type TabType = 'chars' | 'layers' | 'metrics' | 'library';
...
export default function Sidebar({
  ...
  onChangeStencilFont,
  backgroundProject,
  onChangeBackgroundProject
}: SidebarProps) {
  const [activeTab, setActiveTab] = useState<TabType>('chars');
  const [importStatus, setImportStatus] = useState<{ type: 'success' | 'error'; message: string } | null>(null);
  const [isClearConfirmOpen, setIsClearConfirmOpen] = useState<boolean>(false);
  const [presetToLoad, setPresetToLoad] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleJsonImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        // Basic schema validations to prevent crashing Node/Canvas
        if (!parsed.id || !parsed.name || !parsed.metrics || !parsed.glyphs) {
          throw new Error('El archivo cargado no contiene un formato de proyecto válido.');
        }
        
        onChangeProject(parsed);
        showStatus('success', `¡Proyecto "${parsed.name}" importado con éxito!`);
      } catch (err: any) {
        showStatus('error', `Error al importar: ${err.message || err}`);
      }
    };
  ...
  const handleBinaryFontImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (event) => {
      try {
        ...
        const fontInstance = opentype.parse(buffer);
        const parsedProj = fontToFontProject(fontInstance, file.name);

        onChangeProject(parsedProj);
        showStatus('success', `¡Fuente "${file.name}" importada correctamente! Se han extraído ${Object.values(parsedProj.glyphs).filter(g => g.shapes.length > 0).length} caracteres vectoriales.`);
      } catch (err: any) {
        console.error('Error al parsear TTF/OTF:', err);
        showStatus('error', `Error al leer fuente: ${err.message || 'Formato de fuente corrupto o incompatible'}`);
      }
    };
  ...
  const handleBackgroundFontImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (event) => {
      try {
        ...
        onChangeBackgroundProject(parsedProj);
        showStatus('success', `¡Fuente de fondo "${file.name}" cargada con éxito!`);
      } catch (err: any) {
        console.error('Error al parsear TTF/OTF de fondo:', err);
        showStatus('error', `Error al leer fuente de fondo: ${err.message || 'Formato de fuente corrupto o incompatible'}`);
      }
    };
    reader.readAsArrayBuffer(file);
    // clear input
  ...
  const handleLoadPreset = (id: string, name: string) => {
    setPresetToLoad({ id, name });
  };

  const handleConfirmLoadPreset = () => {
    if (!presetToLoad) return;
    const presetProj = createPremadeProject(presetToLoad.id);
    onChangeProject(presetProj);
    showStatus('success', `¡Plantilla "${presetToLoad.name}" cargada correctamente!`);
  ...
  const handleConfirmClearProject = () => {
    ...
    Object.keys(newGlyphs).forEach(char => {
      newGlyphs[char].shapes = [];
    });
    onChangeProject({ ...project, glyphs: newGlyphs, updatedAt: Date.now() });
    onSelectShapes([]); // Clear selected shapes in UI
    showStatus('success', '¡Proyecto limpiado con éxito!');
  };
  ...
  const spanishUpper = new Set(['Ñ', 'Á', 'É', 'Í', 'Ó', 'Ú']);
  const spanishLower = new Set(['ñ', 'á', 'é', 'í', 'ó', 'ú']);

  const uppercaseChars = Object.keys(project.glyphs).filter(c => (c >= 'A' && c <= 'Z') || spanishUpper.has(c));
  const lowercaseChars = Object.keys(project.glyphs).filter(c => (c >= 'a' && c <= 'z') || spanishLower.has(c));
  ...
  return (
    <div id="sidebar-panel" className="w-[280px] border-r border-[#333] bg-[#1a1a1a] flex flex-col h-full text-gray-300 select-none">
      ...
      <div className="grid grid-cols-4 border-b border-[#333] text-[9.5px] text-center font-bold bg-[#141414]">
        <button
          id="tab-btn-chars"
          onClick={() => setActiveTab('chars')}
          className={`py-2 px-0.5 flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer border-r border-[#333] ${
            activeTab === 'chars' ? 'border-b-2 border-b-orange-500 text-orange-500 bg-[#1e1e1e]' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Type size={11} />
          <span>Letras</span>
        ...
        <button
          ...
          className={`py-2 px-0.5 flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer border-r border-[#333] ${
            activeTab === 'layers' ? 'border-b-2 border-b-orange-500 text-orange-500 bg-[#1e1e1e]' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Layers size={11} />
        ...
        <button
          ...
          className={`py-2 px-0.5 flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer border-r border-[#333] ${
            activeTab === 'metrics' ? 'border-b-2 border-b-orange-500 text-orange-500 bg-[#1e1e1e]' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Sliders size={11} />
          <span>Ajustes</span>
        ...
        <button
          ...
          className={`py-2 px-0.5 flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer ${
            activeTab === 'library' ? 'border-b-2 border-b-orange-500 text-orange-500 bg-[#1e1e1e]' : 'text-gray-400 hover:text-white'
          }`}
        >
          <Folder size={11} />
      ...
      <div className="flex-1 overflow-y-auto p-3 custom-scrollbar space-y-3">
        ...
        {activeTab === 'chars' && (
          <div className="space-y-3">
            {/* Category: Uppercase */}
            <div>
              <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest block mb-1.5 font-mono">Mayúsculas</span>
            ...
            {/* Category: Lowercase */}
            <div>
              <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest block mb-1.5 font-mono">Minúsculas</span>
              <div className="grid grid-cols-5 gap-1">
        ...
        {/* TAB 4: PRE-MADE TEMPLATES & PROJECT IMPORT/EXPORT */}
        {activeTab === 'library' && (
          <div className="space-y-4 pb-2 select-text text-gray-300">
            {/* Status alerts */}
            {importStatus && (
              <div id="import-status-banner" className={`p-2 rounded text-[11px] leading-snug animate-in fade-in duration-200 ${
            ...
            {/* Import Font Files & Backups Section */}
            <div className="bg-[#141414] p-3 rounded border border-[#333] space-y-2.5">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block font-mono">Importar Fuente o Proyecto</span>
              <p className="text-[10.5px] text-gray-400 leading-normal">
                Puedes importar tus propias fuentes para editarlas, o un archivo <strong className="text-orange-500 font-bold">.json</strong> de backup.
              </p>
              
              <div className="grid grid-cols-1 gap-2">
                {/* Custom TTF / OTF / WOFF import */}
                <label 
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 border border-dashed border-[#444] hover:border-orange-500 rounded text-xs text-gray-300 hover:text-white bg-[#1a1a1a] cursor-pointer transition-all"
                >
                  <Folder size={12} className="text-orange-550 animate-pulse" />
                  <span>Cargar Fuente (.ttf, .otf, .woff)</span>
                  <input 
                    id="import-binary-font-input"
                    type="file" 
                    accept=".ttf,.otf,.woff" 
                    onChange={handleBinaryFontImport} 
                    className="hidden" 
                  />
                </label>
                ...
                {/* JSON import backup */}
                <label 
                  className="w-full flex items-center justify-center gap-1.5 py-1 px-3 border border-[#2e2e2e] hover:border-gray-500 rounded text-[10.5px] text-gray-400 hover:text-white bg-[#181818] cursor-pointer transition-all"
                >
                  <Plus size={10} className="text-gray-500" />
                  <span>Cargar JSON de Configuración</span>
                  <input 
            ...
            {/* Background font template loader */}
            <div className="bg-[#141414] p-3 rounded border border-[#333] space-y-2">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block font-mono">Plantilla de Fondo</span>
              <p className="text-[10.5px] text-gray-400 leading-normal">
                Permite importar una fuente externa (.ttf, .otf, .woff) para proyectarla como guía semitransparente detrás de tus trazos en el lienzo.
              </p>

              {backgroundProject ? (
                <div className="flex items-center justify-between p-2 rounded bg-orange-950/20 border border-orange-500/20 text-xs mt-1.5">
                  <span className="text-orange-400 font-mono font-medium truncate max-w-[150px]" title={backgroundProject.name}>
                    Fondo: {backgroundProject.name}
                  </span>
                  <button
                    onClick={() => onChangeBackgroundProject(null)}
                    className="p-1 px-2 hover:bg-orange-600 bg-orange-700/80 border border-orange-500/20 text-white rounded text-[9.5px] font-bold cursor-pointer transition"
                  >
                    Quitar
                  </button>
                </div>
                ...
                <label 
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 border border-dashed border-[#555] hover:border-[#aaa] rounded text-xs text-gray-400 hover:text-white bg-[#1a1a1a] cursor-pointer transition-all"
                >
                  <Eye size={12} className="text-gray-400" />
                  <span>Cargar de Fondo (.ttf, .otf, .woff)</span>
                  <input 
                    id="import-bg-font-input"
                    type="file" 
                    accept=".ttf,.otf,.woff" 
                    onChange={handleBackgroundFontImport} 
                    className="hidden" 
                  />
                </label>
              )}
            ...
            {/* Pre-made Library Presets section */}
            <div className="space-y-2">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block font-mono">Librería de Plantillas</span>
              <p className="text-[10.5px] text-gray-400 leading-normal">
                Carga un preset pre-diseñado para ver cómo se compone una tipografía. Al cargar una plantilla, se aplicarán vectores a las siguientes letras:
              </p>

              <div className="space-y-4">
                <button
                  onClick={handleClearProject}
                  className="w-full py-2 text-center bg-[#222] hover:bg-red-900/50 text-gray-300 hover:text-white border border-[#333] hover:border-red-800 text-[11px] font-bold transition rounded cursor-pointer mt-1"
                >
                  Limpiar Proyecto (Vaciar todas las letras)
                </button>
              </div>

              <div className="space-y-2 relative">
                {PREMADE_FONTS.map(font => {
                  // Get designed letters for description
                  const chars = font.id === 'geometric' ? 'A, E, F, H, I, L, O, T, o, X' :
                                font.id === 'blocky' ? 'I, H, O, E, L, T, A' : 'I, H, T, L, E, O, X';
                  return (
                    <div 
                      key={font.id} 
                      id={`preset-card-${font.id}`}
                      className="p-2.5 bg-[#141414]/90 border border-[#2e2e2e] hover:border-orange-550/30 rounded transition-all space-y-2 flex flex-col justify-between animate-in fade-in slide-in-from-bottom-2 duration-200"
                    >
                      <div>
                        <div className="flex items-center justify-between">
                          <h4 className="text-[11px] font-bold text-orange-400 font-mono">{font.name}</h4>
                          <span className="text-[8.5px] bg-orange-950/40 text-orange-400 px-1.5 py-0.5 rounded font-mono font-bold border border-orange-900/40">Vectorial</span>
                        </div>
                        <p className="text-[10px] text-gray-400 mt-1 leading-normal">{font.desc}</p>
                        
                        <div className="mt-2 text-[9px] bg-[#121212] px-2 py-1 rounded border border-[#222] font-mono text-gray-500">
                          <strong className="text-gray-400 font-sans">Letras diseñadas:</strong> {chars}
                        </div>
                      </div>
                      <button
                        id={`load-preset-btn-${font.id}`}
                        onClick={() => handleLoadPreset(font.id, font.name)}
                        className="w-full py-1 text-center bg-[#222] hover:bg-orange-600 text-gray-300 hover:text-white border border-[#333] hover:border-transparent text-[10px] font-bold transition rounded cursor-pointer mt-1"
                      >
                        Cargar Plantilla
                      </button>
                    </div>
      ...
      <ConfirmModal
        ...
        message="¿Seguro que deseas limpiar todas las formas de este proyecto? Esto dejará todas las letras del proyecto en blanco de forma permanente."
      ...
      <ConfirmModal
        isOpen={presetToLoad !== null}
        onClose={() => setPresetToLoad(null)}
        onConfirm={handleConfirmLoadPreset}
        title="Cargar Plantilla"
        message={`¿Seguro que deseas cargar la plantilla "${presetToLoad?.name}"? Esto reemplazará y eliminará tu tipografía actual de forma irreversible.`}
        confirmText="Cargar Plantilla"
        cancelText="Cancelar"
        type="warning"
      />
    </div>
