...
interface CanvasEditorProps {
  ...
  onChangeZoom: (updater: (z: number) => number) => void;
...
export default function CanvasEditor({
  ...
  useEffect(() => {
    ...
    if (isMouseDown) {
      if (dragAction === 'drawShape') {
        ...
        ctx.save();
        ctx.strokeStyle = 'rgba(239, 104, 0, 0.65)'; // Semi-transparent orange marquee
        ctx.fillStyle = 'rgba(239, 104, 0, 0.08)'; // Very faint fill
        ctx.lineWidth = 1;
        ctx.setLineDash([4, 4]);
        
        const rectW = pCurrent.x - pStart.x;
        const rectH = pCurrent.y - pStart.y;
        
        ctx.fillRect(pStart.x, pStart.y, rectW, rectH);
        ctx.strokeRect(pStart.x, pStart.y, rectW, rectH);
        ctx.restore();
      }
    }
  ...
  const distanceToSegment = (p: Point2D, a: Point2D, b: Point2D) => {
    const abX = b.x - a.x;
    const abY = b.y - a.y;
    const abLen2 = abX * abX + abY * abY;
    if (abLen2 === 0) {
      const d = Math.hypot(p.x - a.x, p.y - a.y);
      return { distance: d, projection: a };
    }
    ...
    const d = Math.hypot(p.x - projection.x, p.y - projection.y);
    return { distance: d, projection };
  };
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // [DISABLED GRABBING ADVANCE LINE DIRECTLY VIA CANVAS AS REQUESTED]
    // The advance width orange line remains visible but is strictly read-only on canvas, modified only via sidebar metrics tab to avoid accidental drags.
    ...
    if (activeTool === 'node') {
      ...
      // [ADD VERTICES ON SEGMENT CLICK]
      // Check if user clicked on any segment of active/unlocked shapes to insert a new vertex!
      let insertedNodeX = 0;
      let insertedNodeY = 0;
      let insertShapeId: string | null = null;
      let insertIndex = -1;

      for (const sh of glyph.shapes) {
        if (!sh.visible || sh.locked) continue;
        const numNodes = sh.nodes.length;
        if (numNodes < 2) continue;

        for (let i = 0; i < numNodes; i++) {
          if (i === numNodes - 1 && !sh.closed) break;
          const nA = sh.nodes[i];
          const nB = sh.nodes[(i + 1) % numNodes];

          const res = distanceToSegment(fontPt, nA, nB);
          const rScr = fontToScreen(res.projection, coordsContext);
          const distScr = distance(screenPt, rScr);

          // Clicked within 10 pixels of the segment
          if (distScr < 10) {
            insertedNodeX = res.projection.x;
            insertedNodeY = res.projection.y;
            insertShapeId = sh.id;
            insertIndex = i + 1; // Insert after node i
            break;
          }
        }
        if (insertShapeId !== null) break;
      }

      if (insertShapeId !== null) {
        const newNode: PathNode = {
          id: generateId(),
          x: Math.round(insertedNodeX),
          y: Math.round(insertedNodeY),
          handleIn: { x: -35, y: 0 },
          handleOut: { x: 35, y: 0 },
          type: 'corner'
        };
        ...
        onSelectShapes([insertShapeId!]);
        setNodeInitialState({ ...newNode });
        setDragAction('moveNode');
        return;
      }
  ...
  const handleMouseUp = () => {
    ...
    if (dragAction === 'select') {
      const x1 = Math.min(mouseDownFontPt.x, mouseCurrentFontPt.x);
      const x2 = Math.max(mouseDownFontPt.x, mouseCurrentFontPt.x);
      const y1 = Math.min(mouseDownFontPt.y, mouseCurrentFontPt.y);
      const y2 = Math.max(mouseDownFontPt.y, mouseCurrentFontPt.y);

      // Verify if the dragged selection rectangle covers real area
      if (Math.abs(x2 - x1) > 6 || Math.abs(y2 - y1) > 6) {
        const newlySelected: string[] = [];
        glyph.shapes.forEach(sh => {
          if (!sh.visible || sh.locked) return;
          // Shape is selected if any of its vertices lie inside the rectangle boundary
          const isNodeBound = sh.nodes.some(node => node.x >= x1 && node.x <= x2 && node.y >= y1 && node.y <= y2);
          if (isNodeBound) {
            newlySelected.push(sh.id);
          }
        });
        
        onSelectShapes(newlySelected);
      }
    }
    ...
    if (dragAction === 'drawShape') {
      ...
      if (Math.hypot(dx, dy) > 10) {
        ...
        if (activeTool === 'rectangle') {
          ...
          const r = Math.round(Math.hypot(dx, dy));
          newShape = createPolygon(x1, y1, r, polygonSides); // Fully custom polygon sides
  ...
  // Capture wheel event on canvas to zoom with Shift Key
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const handleWheel = (e: WheelEvent) => {
      if (e.shiftKey) {
        e.preventDefault();
        // Zoom in if moving up, zoom out if moving down
        const delta = -e.deltaY;
        if (delta > 0) {
          onChangeZoom(z => Math.min(4.0, z + 0.08));
        } else if (delta < 0) {
          onChangeZoom(z => Math.max(0.2, z - 0.08));
        }
      }
    };
      ...
      {isTipExpanded ? (
        <div className="absolute bottom-4 left-4 max-w-xs bg-[#1a1a1a]/95 backdrop-blur p-3 rounded border border-[#333] text-[11px] text-gray-400 shadow-xl select-none">
          ...
          <p className="leading-relaxed mb-2 text-gray-300">
            Ajusta el <strong className="text-orange-500">Separación Horizontal</strong> de la letra desde la pestaña <strong className="text-orange-400">Medidas</strong> en la barra lateral.
          </p>
          <div className="border-t border-[#333]/65 pt-2 grid grid-cols-2 gap-2 font-mono text-[9px] text-[#aaa]">
            <div><kbd className="bg-[#2a2a2a] px-1 py-0.5 rounded text-gray-300 border border-[#444]">Shift+Rueda</kbd> Zoom suave</div>
            <div><kbd className="bg-[#2a2a2a] px-1 py-0.5 rounded text-gray-300 border border-[#444]">Clic en Línea</kbd> Añadir vértice</div>
            <div><kbd className="bg-[#2a2a2a] px-1 py-0.5 rounded text-orange-400 border border-[#ff6b00]/30">Shift+Clic</kbd> Multiselección</div>
            <div><kbd className="bg-[#2a2a2a] px-1 py-0.5 rounded text-red-400 border border-red-900/40">Supr/Backspace</kbd> Borrar figura</div>
          </div>
        </div>
      ) : (
        <button 
